...
def get_parent_id_reference(path: str) -> str:
    ...
    return f'(SELECT category_id FROM categories WHERE hierarchy_path = \'{escaped_path}\')'
...
def is_leaf_category(path: str, all_paths: set) -> bool:
    """
    Determine if a category is a leaf (has no children).
    Skip root level categories (e.g., '1', '2', '3').
    A category is a leaf if no other path starts with it followed by a dot.
    """
    # Skip root level categories (single number like '1', '2', etc.)
    if '.' not in path:
        return False
    
    # Check if any path starts with this path followed by a dot
    path_prefix = path + '.'
...
def generate_postgresql_inserts(arabic_file: str, kurdish_file: str) -> str:
    ...
    for path in sorted_paths:
        if is_leaf_category(path, common_paths):
            leaf_categories.add(path)
    
    logger.info(f"Found {len(leaf_categories)} leaf categories (insertable)")
    ...
    for path in sorted_paths:
        ...
        is_leaf = 'TRUE' if path in leaf_categories else 'FALSE'
        
        # Generate INSERT statement
        insert_sql = f"""INSERT INTO categories (name_arabic, name_kurdish, url_slug_arabic, url_slug_kurdish, hierarchy_path, parent_id, is_leaf) 
VALUES ('{arabic_name}', '{kurdish_name}', '{arabic_slug}', '{kurdish_slug}', '{path}', {parent_id}, {is_leaf});"""
